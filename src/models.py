...
fromkeras import optimizers, objectives
...
class KerasModel(Model):
    ...
    def compile(self, optimizer = 'sgd', loss = 'mse', policy_rule = 'max', sample_weight_mode =None):
        ...
        test_loss = weighted_loss(self.y, self.y_test, welf.weights, mask)

        for r in kmodel.regularizers:
            train_loss = r(train_loss)
        updates = self.optimizer.get_updates(kmodel.trainable_weights, kmodel.constraints, train_loss)
        ...
        self._values_train = K.function(values_ins_train, [values_train], updates = kmodel.state_updates)
        self._values_test = K.function(values_ins_test, [values_test], updates = kmodel.state_updates)

        self._test = K.function(test_ins, [test_loss], updates - kmodel.state_updates)
...
class TableModel(Model):
    ...
    def update(self, inputs, targets, actions, weights = None):
        ...
        for i, input in enumerate(inputs):
            a = int(actions[i])
            self.table[tuple(input)][a] = current_values[i][a] + self.lr * (targets[i] - current_values[i][a])
