...
class Model(object):
    ...
    def load_data(self, quantity, start=None, end=None, period=14400, is_pkld=True, test=False):
        ...
        if is_pkld:
            ...
            for i in self.symbols:
                c = pd.DataFrame.from_records(Poloniex(key=polo_api, secret=polo_secret).returnChartData(
    ...
    def fit(self, epochs=100):
        ...
        ), ' (', progressbar.ETA(), ') '], max_value=epochs * quantity).start()
        ...
        for i in range(epochs):
            ...
            if (i % 10 == 0):
                self.rnn.save('../data/model{}sgd.h5'.format(i),
                              overwrite=True)
                #     self.test(ep = i)
    ...
    def test(self, ep):
        ...
        while not term:
            ...
            if term:
                ...
                for i, s in enumerate(self.symbols):
                    scaler = joblib.load(
                        '../data/{}_close_standardScaler.pkl'.format(s))
                    close = scaler.inverse_transform(closeT)
                    b_test = bt.Backtest(pd.Series(data=close[i, :], index=np.arange(len(trades))), pd.Series(
                        data=[j[i] for j in trades]), initialCash=self.init_cash, roundShares=False, signalType='shares')
                    b_test.data['delta'] = b_test.data['shares'].diff().fillna(
                        0)
                    plt.figure(figsize=(12, 8))
                    plt.subplot(2, 1, 1)
                    b_test.plotTrades()
                    ...
                    plt.savefig('../images/{}_{}TEST_summary.png'.format(
                        self.symbols[i], ep), bbox_inches='tight', pad_inches=0.5, dpi=60)
                    plt.close('all')
                    self.init_cash = 1000
                    joblib.dump(
                        b_test.data, '../data/btest_test_{}{}.pkl'.format(s, i))
...
if __name__ == '__main__':
    ...
    model.load_data((end - start) / 14400, start=start, end=end,
                    period=14400, test=False, is_pkld=True)
