...
class Model(object):
    ...
    def predict_for_api(self, num, funds, risk = 0.5):
        ...
        start = end - (604800/4)
        period = 300
        self.load_data((end-start)/period, start = start, end = end, period = period, is_pkld = False, test = True)
    ...
    def bestow(self, new_state, step, action, prices, trades, term, qs = None, epoch = None , plot = False, evaluate = False):
        ...
        if evaluate == False:
            for i in range(len(self.symbols)):
                # ipdb.set_trace()
                b_test = bt.Backtest(pd.Series(data=prices[i,step-2:step], index =np.arange(step-2,step)).astype(float),pd.Series(data = trades[step-2:step,i], index = np.arange(step-2,step)).astype(float),roundShares = False, signalType='shares')
    ...
    def fit(self, epochs = 100):
        ...
        self.learning_progress = [] #Stores state, Action, reward (bestowal :) ) and next state
        ...
        for i in range(epochs):
            ...
            while go:
                ...
                # if memory is full, overwrite values
                    ...
                    for z, mem in enumerate(batch):
                        ...
                        max_mem_q = np.max(self.rnn.predict_on_batch(mem_state_prime), axis = 2)
                        if term:
                            update = [mem_bestowal]
                        else:
            ...
            # import ipdb; ipdb.set_trace()
            print('\n\nEPOCH: {} \nREWARD: {}\nEXPLORATION COEFFICENT: {}\n\n'.format(i+1, epoch_bestowal, explore))
        ...
        plt.plot(self.learning_progress[:,0], self.learning_progress[:,1], label = 'Reward per Epoch', color = 'b', alpha = 0.8)
        plt.plot(self.learning_progress[:,0],moving_average(self.learning_progress[:,1]), label = 'moving average', color = 'k')
