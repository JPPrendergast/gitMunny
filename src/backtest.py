    ...
    def __init__(self, price, signal, signalType='capital', initialCash=0, initialShares=2, roundShares=True, makerFee=0.0015, takerFee=0.0025):
        ...
        *roundShares* round off number of shares to integers
        ...
        assert signalType in [
            'capital', 'shares'], "Wrong signal type provided, must be 'capital' or 'shares'"
        ...
        self.data = pd.DataFrame(index=price.index, columns=[
                                 'price', 'trades', 'shares', 'value', 'cash', 'total_fees', 'pnl', 'netProfit'])
        ...
        if len(signal) > 10:
            ...
            self.data['cash'] = (-self.data['price'] * self.data['trades'] -
                                 self.fees).fillna(0).cumsum() + initialCash
            self.data['netProfit'] = self.data.cash + \
                self.data.value - (self.data.cash[0] + self.data.value[0])
            self.data['pnl'] = self.data['cash'] + self.data['value'] - \
                initialCash - (self.data.price * initialShares)
            # import ipdb; ipdb.set_trace()

    @property
    def sharpe(self):
        ''' return annualized sharpe ratio of the pnl '''
        pnl = (self.data['pnl'].diff()).shift(-1)[self.data['shares']
                                                  != 0]  # use only days with position.
        # need the diff here as sharpe works on daily returns.
        return sharpe(pnl)
